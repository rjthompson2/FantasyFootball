...
def qb_data(year):
    ...
    ws = WebScraper()
    qb_df = bd.player_data(
        "https://www.fantasypros.com/nfl/stats/{position}.php?year="
        + str(year)
        + "&scoring=PPR",
        "data",
        ws,
...
def flex_data(year):
    ...
    flex_df.to_csv(
        find_in_data_folder("HistoricData/wr-te_opportunity_" + str(year) + ".csv"),
        header=True,
        index=False,
    )
